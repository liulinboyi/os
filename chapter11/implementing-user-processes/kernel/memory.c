...
struct pool {
   ...
   struct lock lock;		 // 申请内存时互斥 新加的部分 在内存申请时做互斥，避免公共资源的竞争。
};
...
// 申请一页内存，并用vaddr映射到该页，也就是说我们可以指定虚拟地址
...
/* 得到虚拟地址映射到的物理地址 */
// 先得到虚拟地址vaddr最终所映射到的物理页框起始地址，
// 也就是在页表中vaddr所在的pte中记录的那个物理页地址，
// 然后再将vaddr的低12位与此值相加，所得的地址和便是vaddr映射的物理地址
